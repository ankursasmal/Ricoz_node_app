...
// get value  according search
              let handelDatafatchSearch=async()=>{
                try{
                  let res=await fetch(`http://localhost:3000/search-data/${searchText}`,{
                    method:'GET'
...
let handelDelete=async(id)=>{
    try{
      // console.log('http://localhost:3000'+'/detete-date'+`${id}`)
        let res=await fetch(`http://localhost:3000/detete-date/${id}`,{
            method:'DELETE',
            credentials:'include'
        });
        let result=await res.json();
       ...
       return (
                <center className="flex items-center justify-center flex-col">
                  <div className="mt-10 flex items-center justify-between min-w-[70vw] rounded-lg bg-slate-800 py-[1.75rem] px-[2rem]">
                   ...
                   <Link to='/register' > <button className="py-[.8rem] px-[1.2rem] bg-gray-200 text-black font-semibold rounded-lg ml-2">Add Topic</button></Link>
                  </div>
                  <input
                type='text'
                 value={searchText}
...
<Link to={`/edit/${val._id}`}> <FaRegEdit className=" text-[1.5rem] ml-[2rem] "/></Link>
</div>
